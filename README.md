...
The standard evaluation metric for avalanche segmentation (pixel-level F1) treats every pixel independently. Two models that achieve the same pixel F1 are considered equivalent. But when we evaluate on **instance-level detection** — "which avalanches did the model find?" — the ranking changes.
...
This supports the methodological argument that pixel F1 is not an appropriate standalone evaluation target for SAR avalanche segmentation, where the operational question is "which avalanches were found" rather than "which pixels were correctly labeled."

## Results

### Pixel-level comparison

Numbers for Gatti are from their published paper. Our model evaluated on the same Tromsø OOD test set.

| Metric | D4-EquiCNN (ours) | Swin-UNet (Gatti, published) |
|---|---|---|
...
Our model has higher recall and F2 than the Swin-UNet. The F1 gap comes from precision (boundary sharpness), consistent with the capacity difference between a 625K CNN and a 2.39M vision transformer.

### Gatti et al. Table 5 — updated with our model

All baseline numbers from Gatti et al. 2026 (published).
...
**Verification:** Fragmentation is symmetric between models (mean 1.05 vs 1.14 components per D3 GT). Center-point matching confirms the ranking at every D-scale. D4 sample size (n=16) precludes a strong claim; D3 (n=72) is well-powered.
...
We explored an architectural decomposition: use the Phase 1 detector to propose avalanche regions, then refine boundaries with a conditional segmenter on proposal crops. Key findings:
...
*Model probability map on a region with mixed D-scales. The model produces high-confidence predictions (yellow) on the D3 deposit (cyan boundary) while the adjacent D2 deposit (red boundary) receives lower, more diffuse probability.*

### D2 detection is bimodal — driven by environment, not size

The 25 D2 deposits in the Tromsø test set show a bimodal detection pattern: 15 are detected with high confidence, 7 are clearly missed. **Deposit size does not predict detection success** — deposits of identical area can have opposite outcomes depending on terrain and SAR viewing geometry.
...
- **Decoder**: 4-stage decoder (standard Conv2d). Note: the best configuration (condition 1) does NOT use skip connections from encoder to decoder — the bottleneck change features alone are sufficient. 4 engineered channels (log-ratio VH/VV, cross-pol post/pre) are injected at each decoder scale via adaptive pooling.
- **Output**: 1-channel logit map → sigmoid → binary prediction at optimized threshold (0.225).

D4 equivariance makes the model exactly invariant to horizontal flips, vertical flips, and 90/180/270 degree rotations. This eliminates 4 of 6 standard geometric augmentations. TTA with these transforms is mathematically redundant for the equivariant layers, but we apply 4-fold TTA at inference because it provides marginal improvement (+0.6 pp F1) — likely from boundary effects at tile edges where the receptive field is truncated.
...
Sliding-window inference with 75% overlap, Gaussian blending, morphological closing (disk radius 3, 1 iteration), and 4-fold TTA:
...
All reported pixel F1 numbers use this inference configuration. Morphological closing was applied for the reported numbers.
